...
st.set_page_config(page_title="APEXAI // Pump & Dump Scanner", page_icon="🔥", layout="wide", initial_sidebar_state="expanded")
...
def check_daily_summary(s):
    ...
    try:
        ...
        msg=(f"📊 **APEX 24h Journal Summary**\n"
             ...
             f"🎯 Win Rate: **{wr:.1f}%**\n"
             f"━━━━━━━━━━━━━━━━━\n"
             f"*Powered by APEXAI Intelligence Terminal*")
        ...
        if s.get('discord_webhook'):
            send_discord(s['discord_webhook'],{
                "title":"📊 APEXAI 24h Journal Summary",
                "color":0x2563eb,
                "description":msg,
                "footer":{"text":f"APEXAI Terminal • {now.strftime('%H:%M UTC')} | {(now + __import__('datetime').timedelta(hours=5)).strftime('%H:%M PKT')}"}
            })
    except: pass
...
def send_gl_discord_alert(webhook_url, sig):
    ...
    try:
        ...
        embed = {
            ...
            'footer': {'text': f'APEXAI — Gainers/Losers Scanner • {sig.get("scan_time","–")} | {sig.get("scan_time_pkt","–")}'}
        }
...
def send_dst_discord_alert(webhook_url, sig):
    ...
    try:
        ...
        embed = {
            ...
            'footer': {'text': f'APEXAI — DEMA+ST Scanner • {sig.get("scan_time","–")} | {sig.get("scan_time_pkt","–")}'}
        }
        requests.post(webhook_url, json={'embeds': [embed]}, timeout=5)
    except:
        pass
...
def render_card(res, is_sniper=False, dual_confirmed=False):
    ...
    ai_badge_html = ''
    if _ai_data:
        _conf = _ai_data.get('trade_confidence', 0)
        _rank = _ai_data.get('rank', 99)
    ...
    # ── Freshness bar ─────────────────────────────────────────────────────────
    _fresh_pct = res.get('_freshness_pct', 100)
    _age_min = res.get('_age_min', 0)
    _fg_note = res.get('_fg_note','')
    _fresh_col = '#059669' if _fresh_pct > 66 else ('#d97706' if _fresh_pct > 33 else '#dc2626')
    freshness_html = (
        '<div style="margin:4px 0 6px 0;">' 
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px;">' 
        '<span style="font-family:monospace;font-size:.55rem;color:var(--muted);">Signal freshness</span>' 
        '<span style="font-family:monospace;font-size:.55rem;color:' + _fresh_col + ';">' + str(_age_min) + 'm ago</span>' 
        '</div>' 
        '<div style="height:3px;background:#e2e8f0;border-radius:2px;">' 
        '<div style="height:3px;width:' + str(int(_fresh_pct)) + '%;background:' + _fresh_col + ';border-radius:2px;transition:width 0.3s;"></div>' 
        '</div>' 
        + ('<div style="font-family:monospace;font-size:.55rem;color:#d97706;margin-top:2px;">⚠ ' + _fg_note + '</div>' if _fg_note else '') +
        '</div>'
    )
            ...
            try:
                ...
                for _feed_url in _rss_feeds:
                    try:
                        ...
                        pass
                if not headlines:
                    headlines = [{'title': 'No news fetched — using APEXAI signal coins for analysis', 'coins': []}]
                ...
                if ak_recheck not in st.session_state.alerted_sigs:
                    ...
                    if eff_s.get('discord_webhook'):
                        ...
                        send_discord(eff_s['discord_webhook'],{
                            ...
                            'footer':{'text':f'APEXAI Intelligence Terminal • {_dual_time()[0]} | {_dual_time()[1]}'}
                        })
...
| Errors | **{len(errs)}** out of {eff_s['scan_depth']} coins |""")
    ...
    for _r in results:
        try:
            ...
            _age_min = 0
        _r['_age_min'] = round(_age_min, 1)
        # decay: -2 pts per minute after 5 min, cap at -20
        _decay = max(0, min(20, (_age_min - 5) * 2)) if _age_min > 5 else 0
        _r['_decayed_score'] = max(0, _r['pump_score'] - _decay)
    ...
    for _r in results:
        _fg_penalty = 0
        _fg_note = ''
        if _fng <= 25 and _r['type'] == 'LONG' and not _r.get('dual_confirmed') and not _r.get('momentum_confirmed'):
            _fg_penalty = 15
            _fg_note = f'F&G Extreme Fear ({_fng}) — unconfirmed LONG penalised'
        elif _fng <= 25 and _r['type'] == 'LONG' and _r.get('momentum_confirmed') and not _r.get('dual_confirmed'):
            _fg_penalty = 7
            _fg_note = f'F&G Extreme Fear ({_fng}) — partial penalty'
        elif _fng >= 75 and _r['type'] == 'SHORT' and not _r.get('dual_confirmed'):
            _fg_penalty = 10
            _fg_note = f'F&G Extreme Greed ({_fng}) — unconfirmed SHORT penalised'
        _r['_fg_penalty'] = _fg_penalty
        _r['_fg_note'] = _fg_note
        _r['_final_score'] = max(0, _r['_decayed_score'] - _fg_penalty)
