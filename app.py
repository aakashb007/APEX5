...
st.set_page_config(page_title="APEX51 // Pump & Dump Scanner", page_icon="🔥", layout="wide", initial_sidebar_state="expanded")
...
JOURNAL_FILE="trade_journal.csv"; COOLDOWN_FILE="symbol_cooldowns.json"; SETTINGS_FILE="APEX51_settings.json"; GL_PERF_FILE="gl_performance.csv"
...
def check_daily_summary(s):
    ...
    try:
        ...
        msg=(f"📊 **APEX51 24h Journal Summary**\n"
             ...
             f"🎯 Win Rate: **{wr:.1f}%**\n"
             f"━━━━━━━━━━━━━━━━━\n"
             f"*Powered by APEX51 Intelligence Terminal*")
        ...
        if s.get('discord_webhook'):
            send_discord(s['discord_webhook'],{
                "title":"📊 APEX51 24h Journal Summary",
                "color":0x2563eb,
                "description":msg,
                "footer":{"text":f"APEX51 Terminal • {now.strftime('%H:%M UTC')} | {(now + __import__('datetime').timedelta(hours=5)).strftime('%H:%M PKT')}"}
            })
    except: pass
...
def run_dst_scan(coin_list, s, exchange_clients):
    ...
    If watchlist is off — uses APEX51 coin list.
    """
    results = []
    ...
    if use_watchlist and watchlist:
        # Build coin list directly from watchlist — independent of APEX51
        coin_list = []
...
def send_gl_discord_alert(webhook_url, sig):
    ...
    try:
        ...
        embed = {
            ...
            'footer': {'text': f'APEX51 — Gainers/Losers Scanner • {sig.get("scan_time","–")} | {sig.get("scan_time_pkt","–")}'}
        }
...
def send_dst_discord_alert(webhook_url, sig):
    ...
    try:
        ...
        embed = {
            ...
            'footer': {'text': f'APEX51 — DEMA+ST Scanner • {sig.get("scan_time","–")} | {sig.get("scan_time_pkt","–")}'}
        }
        requests.post(webhook_url, json={'embeds': [embed]}, timeout=5)
    except:
        pass
    ...
    def fetch_reddit_buzz(self,coin_sym,s):
        ...
        try:
            ...
            rr=requests.get(f"https://www.reddit.com/r/{subs}/search.json",
                ...
                headers={"User-Agent":"Mozilla/5.0 APEX51/3.0"},timeout=6)
    ...
    async def analyze(self,exch_name,exch_obj,sym,s,btc_trend):
        ...
        if dst and dst['direction'] == sig:
            ...
            result['reasons'].insert(0,
                f"🔵 DEMA+ST CONFIRMED: Both APEX51 + DEMA/SuperTrend agree {sig} "
            ...
            result['warnings'].append(
                f"⚠️ DEMA+ST conflict: APEX51 says {sig} but DEMA/SuperTrend says {dst['direction']}")
...
# ─── HEADER / TICKER ─────────────────────────────────────────────────────────
st.markdown('<div style="padding:18px 0 14px;"><div style="font-family:monospace;font-size:1.5rem;font-weight:700;color:#0f1117;">APEX51</div><div style="font-family:monospace;font-size:.56rem;font-weight:400;letter-spacing:.16em;color:#7a82a0;text-transform:uppercase;margin-top:2px;">Pump & Dump Intelligence Terminal v3.0 — Dual Confirm + FVG + Backtest</div></div>',unsafe_allow_html=True)
...
if nav=="⚙️ Settings":
    ...
    with st.form("settings_form"):
        ...
        st.markdown('<div class="hint">Runs alongside APEX51 on every scan. When both strategies agree → score boost. When they conflict → small penalty. Disable to skip entirely.</div>', unsafe_allow_html=True)
        ...
        with dst_c3:
            ...
            st.markdown('<div class="setting-help">DST TP/SL shown in confirm reason only. APEX51 TP/SL is unchanged.</div>', unsafe_allow_html=True)
        with dst_c4:
            st.markdown("**Scoring**")
        ...
        st.markdown('<div class="setting-help">When ON — only scans coins below. When OFF — scans all APEX51 coins.</div>', unsafe_allow_html=True)
        ...
        if ns_dst_watchlist:
            ...
            st.markdown('<div style="font-family:monospace;font-size:.6rem;color:#94a3b8;">No coins selected — DST will scan all APEX51 coins</div>', unsafe_allow_html=True)
        ...
        with gl_s0a:
            ...
            st.markdown('<div class="setting-help">Coins in this list will be excluded from ALL scans (APEX51 + G/L). Commodities like XAU, XAG, SILVER, UKOIL are auto-filtered — add any extras here.</div>', unsafe_allow_html=True)
        with gl_s0b:
...
if nav=="📒 Journal":
    ...
    if df_j.empty:
        ...
        st.download_button("⬇️ Export CSV",dv.to_csv(index=False).encode(),
            file_name=f"APEX51_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv")
...
if nav=="📊 Backtest":
    ...
    if bt_mode=="📂 Upload External CSV":
        ...
        if st.button("▶️ RUN EXTERNAL BACKTEST",use_container_width=True):
            ...
            st.download_button(
                ...
                file_name=f"APEX51_external_backtest_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv")
    ...
    if st.button("▶️ RUN BACKTEST",use_container_width=True):
        ...
        st.download_button(
            ...
            file_name=f"APEX51_backtest_{datetime.now().strftime('%Y%m%d')}.csv",
            mime="text/csv")
...
if nav=="🧠 Catalyst":
    ...
    st.markdown('<div style="font-family:monospace;font-size:.62rem;color:#64748b;margin-bottom:12px;">Powered by Groq LLaMA 3.1 · Identifies narrative-driven pump catalysts · Cross-validates with APEX51 signals</div>', unsafe_allow_html=True)
    ...
    if cat_should_run:
        with st.spinner("🧠 Fetching news and analyzing catalysts..."):
            try:
                # ── Fetch news from CoinGecko trending (free, no key) ─
                import urllib.request, json as _json
                headlines = []
                try:
                    trending_url = "https://api.coingecko.com/api/v3/search/trending"
                    with urllib.request.urlopen(trending_url, timeout=10) as r:
                        trending_data = _json.loads(r.read())
                ...
                for _feed_url in _rss_feeds:
                    try:
                        ...
                        pass
                if not headlines:
                    headlines = [{'title': 'No news fetched — using APEX51 signal coins for analysis', 'coins': []}]

                # ── Get current APEX51 signal coins ─────────────────────
                APEX51_coins = list(set([
                ...
                APEX51_text = ", ".join(APEX51_coins) if APEX51_coins else "BTC, ETH, SOL, BNB, ARB"
...
COINS CURRENTLY IN APEX51 SCANNER: {APEX51_text}
...
APEX51_match should be true if the coin symbol appears in: {APEX51_text}
red_flag should be true only for clear shill/spam/pump-and-dump patterns"""
    ...
    if cat_results:
        ...
        for i, coin in enumerate(cat_results):
            ...
            bar_w = min(score, 100)
            APEX51_match = coin.get('APEX51_match', False)
            ...
            APEX51_badge = (
                ...
                'background:#f0fdf4;color:#059669;border:0.5px solid #059669;">✅ APEX51 match active</span>'
                if APEX51_match else
                '<span style="font-family:monospace;font-size:11px;color:#94a3b8;">No active APEX51 signal</span>'
            )
            flag_badge = (
                '<span style="font-family:monospace;font-size:11px;padding:2px 8px;border-radius:4px;'
            ...
            _cat_html = (
                ...
                + flag_badge +
                '</div>'
                '<div style="border-top:1px solid #e2e8f0;padding-top:8px;">' + APEX51_badge + '</div>'
                '</div>'
            )
            with card_cols[i % 2]:
                st.markdown(_cat_html, unsafe_allow_html=True)

        # ── Cross-validation on APEX51 results ──────────────────────────
        st.markdown("---")
        st.markdown('<div style="font-family:monospace;font-size:.75rem;font-weight:700;color:#64748b;">Cross-validation — coins appearing in both APEX51 and Catalyst</div>', unsafe_allow_html=True)
        APEX51_confirmed = [c for c in cat_results if c.get('APEX51_match') and not c.get('red_flag') and c.get('pump_probability') in ['High','Medium']]
        if APEX51_confirmed:
            for coin in APEX51_confirmed:
            ...
            st.markdown('<div style="font-family:monospace;font-size:.65rem;color:#94a3b8;">No cross-validated signals yet — run APEX51 scan first then run Catalyst scan</div>', unsafe_allow_html=True)
                ...
                if ak_recheck not in st.session_state.alerted_sigs:
                    ...
                    if eff_s.get('discord_webhook'):
                        ...
                        if _dst_confirmed and _dst:
                            ...
                            dst_line = (f"\n🔵 **DEMA+ST CONFLICT** ⚠️ | DEMA+ST says {_dst['direction']} (opposite to APEX51)")
                        elif not _dst:
                        ...
                        send_discord(eff_s['discord_webhook'],{
                            ...
                            'footer':{'text':f'APEX51 Intelligence Terminal • {_dual_time()[0]} | {_dual_time()[1]}'}
                        })
                ...
                if ak not in st.session_state.alerted_sigs:
                    ...
                    if eff_s.get('discord_webhook'):
                        ...
                        send_discord(eff_s['discord_webhook'],{
                            ...
                            "footer":{"text":f"APEX51 Sentinel — Top 100 | {_dual_time()[0]} | {_dual_time()[1]}"}})
...
if dst_should_run:
    ...
    if not coin_list:
        st.warning("⚠️ Run an APEX51 scan first so DEMA+ST has a coin list to work with.")
    else:
